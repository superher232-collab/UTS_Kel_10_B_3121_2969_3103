...
function FleetContent() {
  // Tambahkan editKapal ke dalam destrukturisasi context
  const { role, armada, tambahKapal, hapusKapal, editKapal, loading } = useDashboard();
  const ships = armada && armada.length > 0 ? armada : [];
  ...
  // State form & modal
  const [showModal, setShowModal] = useState(false);
  const [isEditing, setIsEditing] = useState(false); // Penanda apakah mode Edit atau Tambah
  const [editId, setEditId] = useState<any>(null); // Menyimpan ID kapal yang diedit
  ...
  // Fungsi untuk membuka modal edit dengan data terisi
  const handleOpenEdit = (ship: any) => {
    setIsEditing(true);
    setEditId(ship.id);
    setForm({
      name: ship.name || '',
  ...
  // Fungsi submit disatukan untuk Tambah & Update
  const handleSubmitForm = async () => {
    ...
    // Panggil fungsi context yang sesuai
    if (isEditing) {
      // Pastikan fungsi editKapal tersedia di DashboardContext Anda
      result = await editKapal(editId, form); 
    } else {
    ...
    if (result && result.success) {
      ...
      // Jika tambah kapal baru, pindah ke halaman terakhir
      if (!isEditing) {
        setSearchTerm('');
        setCurrentPage(Math.ceil((ships.length + 1) / itemsPerPage) || 1);
      }
    } else {
      alert(`Gagal ${isEditing ? 'mengupdate' : 'menambah'} kapal. Cek console untuk detail.`);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '100%', color: 'white', fontFamily: 'monospace' }}>
          ...
          <div style={{ background: '#130a24', border: '1px solid rgba(168, 85, 247, 0.3)', borderRadius: '8px', padding: '32px', width: '100%', maxWidth: '480px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
              ...
              <button
                ...
                style={{ flex: 1, background: '#A855F7', border: 'none', color: 'white', padding: '10px', borderRadius: '4px', cursor: loadingForm ? 'not-allowed' : 'pointer', fontSize: '12px', fontWeight: 'bold', opacity: loadingForm ? 0.7 : 1 }}
              >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
        ...
        {totalFilteredShips === 0 && (
          <div style={{
            ...
            padding: '64px 24px', background: 'var(--bg-card, #130a24)', border: '1px dashed rgba(168, 85, 247, 0.3)',
            borderRadius: '8px', textAlign: 'center', gap: '16px', gridColumn: '1 / -1'
          }}>
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgba(168, 85, 247, 0.5)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="11" cy="11" r="8"></circle>
              <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
            ...
            <button
              ...
              style={{
                ...
                padding: '8px 16px', borderRadius: '4px', fontSize: '11px', cursor: 'pointer', fontWeight: 'bold', transition: 'background 0.2s'
              }}
              onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(168, 85, 247, 0.2)'}
              onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(168, 85, 247, 0.1)'}
            >
      ...
      {/* Pagination Footer */}
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', marginTop: '12px', paddingTop: '16px', borderTop: '1px solid rgba(168, 85, 247, 0.15)' }}>
          <div style={{ fontSize: '11px', color: 'var(--text-muted, #8B7BA8)' }}>
            Menampilkan <span style={{ color: 'white', fontWeight: 'bold' }}>{startIndex + 1}</span> - <span style={{ color: 'white', fontWeight: 'bold' }}>{Math.min(startIndex + itemsPerPage, totalFilteredShips)}</span> dari <span style={{ color: 'white', fontWeight: 'bold' }}>{totalFilteredShips}</span> armada
          </div>

          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} style={{ background: currentPage === 1 ? 'rgba(255,255,255,0.02)' : 'rgba(168, 85, 247, 0.1)', border: `1px solid ${currentPage === 1 ? 'rgba(255,255,255,0.05)' : 'rgba(168, 85, 247, 0.3)'}`, color: currentPage === 1 ? 'rgba(255,255,255,0.2)' : 'white', padding: '6px 12px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: currentPage === 1 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg> Sebelumnya
            </button>

            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
              <button key={page} onClick={() => setCurrentPage(page)} style={{ background: currentPage === page ? 'linear-gradient(90deg, #A855F7 0%, #7C3AED 100%)' : 'rgba(20, 10, 36, 0.6)', border: `1px solid ${currentPage === page ? '#A855F7' : 'rgba(168, 85, 247, 0.2)'}`, color: 'white', width: '28px', height: '28px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {page}
              </button>
            ))}

            <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} style={{ background: currentPage === totalPages ? 'rgba(255,255,255,0.02)' : 'rgba(168, 85, 247, 0.1)', border: `1px solid ${currentPage === totalPages ? 'rgba(255,255,255,0.05)' : 'rgba(168, 85, 247, 0.3)'}`, color: currentPage === totalPages ? 'rgba(255,255,255,0.2)' : 'white', padding: '6px 12px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: currentPage === totalPages ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
              Selanjutnya <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
            </button>
          </div>
        </div>
      )}
