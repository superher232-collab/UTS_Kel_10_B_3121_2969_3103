...
// ─────────────────────────────────────────────────────────────
// TYPE DEFINITIONS (Biar nggak pakai 'any' lagi)
// ─────────────────────────────────────────────────────────────
type ShipStatus = "DALAM PERJALANAN" | "DI PELABUHAN" | "TERLAMBAT" | "PEMELIHARAAN";
type WeatherType = "Badai Ekstrem" | "Terik Gersang" | "Normal" | null;
...
// ─────────────────────────────────────────────────────────────
// MOCK DATA (Fallback kalau API belum ready / context belum ada)
// ─────────────────────────────────────────────────────────────
const MOCK_ARMADA: Ship[] = [
  {
    id: 1,
    name: "KM BIMA SAKTI",
    status: "DI PELABUHAN",
...
const MOCK_LOGS: LogEntry[] = [
  {
    id: 1,
    vesselName: "KM BIMA SAKTI",
    event: "Berangkat",
    notes: "Muatan lengkap, cuaca normal",
    status: "ON TIME",
    timestamp: new Date().toISOString(),
  },
  {
    id: 2,
    vesselName: "KM SRIWIJAYA",
    event: "Update Posisi",
    notes: "Melewati Selat Sunda",
    status: "NORMAL",
    timestamp: new Date().toISOString(),
  },
];
...
function DashboardContent() {
  ...
  // State lokal (nanti bisa diganti pakai context)
  const [role] = useState<"Admin" | "Customer">("Customer"); // Nanti ambil dari session
  const [cuaca, setCuaca] = useState<WeatherType>("Normal");
  const [armada, setArmada] = useState<Ship[]>([]);
  ...
  // ───────────────────────────────────────────────────────────
  // FETCH DATA DARI API (Nanti ganti endpoint beneran)
  // ───────────────────────────────────────────────────────────
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Nanti ganti dengan fetch('/api/kapal') atau context
        const res = await fetch("/api/kapal").catch(() => null);
        ...
        if (res?.ok) {
          ...
          if (data.status === "success") {
            ...
            setArmada(MOCK_ARMADA);
          }
        } else {
          setArmada(MOCK_ARMADA); // Fallback ke mock data
        }
        setLogs(MOCK_LOGS);
  ...
  // ───────────────────────────────────────────────────────────
  // MONITOR STATUS PERUBAHAN KAPAL → TRIGGER ALERT
  // ───────────────────────────────────────────────────────────
  useEffect(() => {
  ...
  // ───────────────────────────────────────────────────────────
  // ANIMASI KAPAL DI PETA (Physics simulation)
  // ───────────────────────────────────────────────────────────
  useEffect(() => {
    if (!armada || armada.length === 0) return;
    ...
    const getShipRouteSettings = (name: string, status: string) => {
      ...
      if (!isMoving) {
        if (shipName.includes("BIMA SAKTI")) { x = 300; y = 360; }
        else if (shipName.includes("SRIWIJAYA")) { x = 260; y = 365; }
        else if (shipName.includes("GADJAH MADA")) { x = 250; y = 175; }
        else if (shipName.includes("DEWARUCI")) { x = 380; y = 370; }
        else { x = 450; y = 375; }
      } else {
        const speed = 0.22;
        ...
        if (shipName.includes("NUSANTARA")) { minX = 280; maxX = 520; minY = 250; maxY = 340; x = 300 + Math.random() * 100; y = 260 + Math.random() * 60; }
        else if (shipName.includes("KARTINI") || shipName.includes("RAJAWALI")) { minX = 550; maxX = 600; minY = 150; maxY = 275; x = 560 + Math.random() * 30; y = 170 + Math.random() * 80; }
        else if (shipName.includes("MAJAPAHIT")) { minX = 250; maxX = 320; minY = 60; maxY = 140; x = 260 + Math.random() * 40; y = 80 + Math.random() * 40; }
        else if (shipName.includes("CENDRAWASIH")) { minX = 740; maxX = 820; minY = 200; maxY = 320; x = 750 + Math.random() * 50; y = 220 + Math.random() * 80; }
        else { minX = 150; maxX = 850; minY = 80; maxY = 420; x = 400 + Math.random() * 200; y = 200 + Math.random() * 100; }
      }
      return { x, y, minX, maxX, minY, maxY, vx, vy };
    };

    shipsRef.current = armada.map((ship) => {
  ...
  const getBadgeStyle = (status: string): React.CSSProperties => {
    ...
    if (text.includes("PERJALANAN")) {
      return { ...base, background: "rgba(34, 197, 94, 0.1)", borderColor: "#22C55E", color: "#22C55E", boxShadow: "0 0 10px rgba(34, 197, 94, 0.2)" };
    } else if (text.includes("PELABUHAN")) {
      return { ...base, background: "rgba(59, 130, 246, 0.1)", borderColor: "#3B82F6", color: "#3B82F6", boxShadow: "0 0 10px rgba(59, 130, 246, 0.2)" };
    } else if (text.includes("TERLAMBAT")) {
      return { ...base, background: "rgba(245, 158, 11, 0.15)", borderColor: "#F59E0B", color: "#F59E0B", boxShadow: "0 0 10px rgba(245, 158, 11, 0.3)" };
    } else {
      return { ...base, background: "rgba(239, 68, 68, 0.15)", borderColor: "#EF4444", color: "#EF4444", boxShadow: "0 0 10px rgba(239, 68, 68, 0.3)" };
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "400px", color: "#A855F7", fontFamily: "monospace", fontSize: "14px" }}>
        <span style={{ animation: "pulse 1.5s infinite" }}>🛰️ Menghubungkan ke satelit PrimeLog...</span>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      {errorSignal && (
        <div style={{
          ...
          <button onClick={playAudioAlert} style={{ background: "white", border: "none", borderRadius: "4px", color: "#B91C1C", padding: "6px 12px", fontSize: "10px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 0 10px rgba(255, 255, 255, 0.3)", fontFamily: "monospace" }}>
            Picu Klakson Bahaya
          </button>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "24px", alignItems: "start" }} className="main-cmd-grid">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div style={{ width: "100%", height: "460px", background: "#0D0618", border: "1px solid rgba(168, 85, 247, 0.35)", borderRadius: "12px", position: "relative", overflow: "hidden", boxShadow: "0 10px 40px rgba(0,0,0,0.6)" }}>
              ...
              {cuaca === "Badai Ekstrem" && (
                <g opacity="0.5">
                  {Array.from({ length: 12 }).map((_, i) => (
                    <line key={i} x1={(i * 90) % 1000} y1={(i * 45) % 500} x2={((i * 90) % 1000) + 10} y2={((i * 45) % 500) + 25} stroke="#A855F7" strokeWidth="1.2" style={{ animation: "rainFlow 1.5s linear infinite" }} />
                  ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "16px" }}>
            {[
              ...
              <div key={m.label} style={{ background: "#0D0618", border: "1px solid rgba(168, 85, 247, 0.2)", borderRadius: "8px", padding: "12px 16px", textAlign: "center", boxShadow: `0 4px 15px ${m.shadow}` }}>
        ...
        <div style={{ background: "#0D0618", border: "1px solid rgba(168, 85, 247, 0.35)", borderRadius: "12px", padding: "24px", boxShadow: "0 10px 40px rgba(0,0,0,0.6)", height: "540px", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: "12px", paddingRight: "4px" }}>
            {armada && armada.map((ship) => (
              <div key={ship.id} style={{ background: "rgba(255, 255, 255, 0.02)", border: "1px solid rgba(168, 85, 247, 0.15)", borderRadius: "6px", padding: "12px", display: "flex", flexDirection: "column", gap: "8px", transition: "border-color 0.2s", cursor: "pointer" }}
                ...
                onClick={() => {
                  if (role === "Admin") {
                    const statuses: ShipStatus[] = ["DALAM PERJALANAN", "DI PELABUHAN", "TERLAMBAT", "PEMELIHARAAN"];
                    const currentIdx = statuses.indexOf(ship.status);
                    const nextStatus = statuses[(currentIdx + 1) % statuses.length];
                    setArmada((prev) => prev.map((s) => (s.id === ship.id ? { ...s, status: nextStatus } : s)));
                    setLogs((prev) => [{ id: Date.now(), vesselName: ship.name, event: "Status Update", notes: `Status diubah menjadi ${nextStatus}`, status: nextStatus, timestamp: new Date().toISOString() }, ...prev].slice(0, 50));
                  } else {
              ...
              >
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", fontSize: "10px", color: "#C7B8EA", rowGap: "4px" }}>
                  ...
                  <span style={{ color: ship.status === "TERLAMBAT" ? "#F59E0B" : "white", fontWeight: ship.status === "TERLAMBAT" ? "bold" : "normal" }}>{ship.eta}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "24px" }}>
        {/* Logs */}
        <div style={{ background: "#0D0618", border: "1px solid rgba(168, 85, 247, 0.35)", borderRadius: "12px", padding: "24px", boxShadow: "0 10px 40px rgba(0,0,0,0.6)" }}>
        ...
        {/* Weather Controls */}
        <div style={{ background: "#0D0618", border: "1px solid rgba(168, 85, 247, 0.35)", borderRadius: "12px", padding: "24px", boxShadow: "0 10px 40px rgba(0,0,0,0.6)", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
          {role === "Admin" ? (
            <>
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", justifyContent: "center" }}>
                {(["Badai Ekstrem", "Terik Gersang", "Normal"] as const).map((c) => (
                  <button key={c} onClick={() => { setCuaca(c); playAudioAlert(); }} style={{
                    background: cuaca === c ? (c === "Badai Ekstrem" ? "#EF4444" : c === "Terik Gersang" ? "#F59E0B" : "#22C55E") : "rgba(255, 255, 255, 0.03)",
                    border: `1px solid ${cuaca === c ? (c === "Badai Ekstrem" ? "#EF4444" : c === "Terik Gersang" ? "#F59E0B" : "#22C55E") : "rgba(168, 85, 247, 0.3)"}`,
                    color: "white",
                    padding: "8px 16px",
                    borderRadius: "4px",
                    fontSize: "10px",
            ...
            <>
              ...
              <div style={{ border: "1px solid rgba(168, 85, 247, 0.3)", padding: "10px 20px", borderRadius: "6px", background: "rgba(168, 85, 247, 0.05)", color: "#C084FC", fontWeight: "bold", fontSize: "12px", fontFamily: "monospace", letterSpacing: "1px" }}>
      ...
      <style>{`
        @keyframes warningPulse { 0%, 100% { box-shadow: 0 0 15px rgba(239, 68, 68, 0.4); } 50% { box-shadow: 0 0 25px rgba(239, 68, 68, 0.7); } }
        @keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }
        @keyframes rainFlow { 0% { transform: translate(-80px, -160px); opacity: 0; } 10% { opacity: 0.55; } 90% { opacity: 0.55; } 100% { transform: translate(120px, 240px); opacity: 0; } }
        @keyframes windFlow { 0% { stroke-dashoffset: 400; } 100% { stroke-dashoffset: 0; } }
        @keyframes stormLightning { 0%, 94%, 100% { opacity: 0; } 95%, 97% { opacity: 0.3; } 96% { opacity: 0.05; } }
        @keyframes radarRotation { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
        @media (max-width: 900px) { .main-cmd-grid { grid-template-columns: 1fr !important; } }
      `}</style>
    </div>
  );
}
